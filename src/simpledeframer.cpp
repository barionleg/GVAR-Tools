...
template <typename T>
inline bool getBitNormal(T data, int bit) {
    return (data >> bit) & 1;
}
...
template <typename SYNC_T, int SYNC_SIZE, int FRAME_SIZE, SYNC_T ASM_SYNC>
std::vector<std::vector<uint8_t>> SimpleDeframer<SYNC_T, SYNC_SIZE, FRAME_SIZE, ASM_SYNC>::work(uint8_t *data, int len)
{
    // Output buffer
    ...
    for (int byten = 0; byten < len; byten++)
    {
        ...
        for (int i = 7; i >= 0; i--)
        {
            ...
            for(int j = 0; j < SYNC_SIZE; j++){
                if(getBit(shifter, j) != getBitNormal(ASM_SYNC, j)) badBits++;
                if(badBits == 3) break;
            }
